...
export const POST: RequestHandler = async ({ request, locals }) => {
	...
	const { articleId, title, content, teaser, thumbnail, thumbnailId, thumbnailBase64, languageCode } = await request.json();
	...
	return json({ articleId, status: 'ok', thumbnail: finalThumbnail });
};
